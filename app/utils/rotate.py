...
def rotation_degree(lines: list) -> float:
    """
...
def rotate_image(angle: float, image_path: Path) -> Path:
    ...
    rotated_image_path = str(image_path).replace(suffix, f'_rotated{suffix}')
    cv2.imwrite(rotated_image_path, rotated_image)
    return Path(rotated_image_path)
